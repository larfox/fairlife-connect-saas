...
import { QueueRefreshContext } from "@/hooks/useQueueRefresh";
...
const QueueManagement = ({ selectedEvent, onBack }: QueueManagementProps) => {
  ...
  // Function to refresh all service queues - used when Know Your Numbers is completed
  const refreshAllQueues = () => {
    console.log('Refreshing all service queues...');
    // Increment refresh counter to force re-render of all service components
    setRefreshCounter(prev => prev + 1);
    fetchQueueStats();
  };

  return (
    <QueueRefreshContext.Provider value={{ refreshAllQueues }}>
